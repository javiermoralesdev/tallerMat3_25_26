...
```
El modelo con mayor R² es el de log(Freq) contra log(Rank) con un R²= `r round(summary(sol3)$r.squared,4)`. La pendiente es `r round(coef(sol3)[2],4)`. Por tanto, la ley de Zipf **se cumple/no se cumple** para la longitud de los comentarios de los apartamentos de Mallorca.
